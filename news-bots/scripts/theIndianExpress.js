...
import { TheIndianExpress } from "../models/news-bots.model.js";
import path from "path";

class TheIndianExpressScraper {
  constructor(browser) {
  ...
  async getFullArticle(href) {
    ...
    const newsArticle = await this.page.evaluate(() => {
      const article = {};
      article.title = document.querySelector("h1.heading-three")?.innerText;
      const articleBlocks = document.querySelectorAll("div.pcl-container p");
  ...
  async loadStoredNews() {
    try {
      ...
      return JSON.parse(data).indianexpress;
    } catch (error) {
      return [];
    }
  }
  ...
  async saveNewsStorage(newNewsArray) {
    ...
    jsonData.indianexpress = newNewsArray;
  ...
  async scrapeNews() {
    try {
      ...
      await this.closePage();
      console.log("Indian Express news scraped successfully.");
    } catch (error) {
...
export default TheIndianExpressScraper;
